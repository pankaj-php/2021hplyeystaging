...
<div class="tabs-container alt">
	...
	<div class="tab-content" id="tab2" style="display: none;">	
		...
		<?php else :
			/*WPEngine compatibility*/
			if (defined('PWP_NAME')) { ?>
				<form class="register" id="register" action="<?php echo $pagelink.'?action_ne=register&wpe-login=';echo PWP_NAME; ?>" method="post">
			<?php } else { ?>
				<form class="register" id="register" action="<?php echo $pagelink.'?action_new=register'; ?>" method="post">  
			<?php } ?>	
				...
				<?php 
	 				...
	 				if ( $privacy_policy_status && function_exists( 'the_privacy_policy_link' ) ) { ?>
						<p class="form-row margin-top-10 margin-bottom-10">
				            <label for="privacy_policy"><input type="checkbox" id="privacy_policy" name="privacy_policy"><?php esc_html_e( 'I agree to the', 'listeo_core' ); ?> <a href="<?php echo get_privacy_policy_url(); ?>"><?php esc_html_e( 'Privacy Policy', 'listeo_core' ); ?></a>    </label>
