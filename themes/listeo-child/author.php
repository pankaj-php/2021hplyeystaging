			...
			<div class="user-profile-titlebar">
				...
				<div class="main-btn-new">
					<a href="#small-dialog" class="send-message-to-owner button popup-with-zoom-anim new-btn-class">
						...
						<?php 
							...
							else esc_html_e('Message Vendor', 'listeo');
				...
				<?php 
					...
					else
					{
						...
						<!-- <p><?php //printf( esc_html__( 'Please %s sign in%s to see contact details.', 'listeo' ), '<a href="#sign-in-dialog" class="sign-in popup-with-zoom-anim">', '</a>' ) ?></p> -->
						<?php
					}
					?>
					...
					<?php
						...
						echo '<p class="em-ic">Email Verified</p>';
		   				...
		   				$total_visitor_reviews_args = array(
		                        ...
		                        $selectCon= $wpdb->get_results("SELECT *  FROM {$wpdb->prefix}listeo_core_conversations WHERE user_2 = {$user->ID}");

	       $alltime = [];
	       foreach($selectCon as $con){
		  $time = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}listeo_core_messages WHERE conversation_id = {$con->id} AND sender_id = {$user->ID} LIMIT 1");
		   if(!empty($time)){
			$alltime[]= $time;
		   }
					...
					<div class="col-lg-12 col-md-12">
						<section class="no-results not-found">
							<div class="page-content">
								<div class="notification notice"><p><?php esc_html_e( 'This user is a Verified Hypley User.', 'listeo' ); ?></p></div>
							</div><!-- .page-content -->
						</section>
					</div>
				<?php endif; ?>
